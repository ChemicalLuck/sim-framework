...
The defining constraint: **`@chemicalluck/sim-engine` is game-agnostic and never imports game code.**
...
Each engine feature under `@chemicalluck/sim-engine/features/<name>/` follows the same layout as an
...
`@chemicalluck/sim-engine` ships as **TypeScript source** — your game's Vite process (via the CLI)
transpiles it. The engine's Node-side tooling (the two plugins) is compiled to JS so the
CLI can import it. The CLI aliases `@chemicalluck/sim-engine` to the engine source directory so deep
